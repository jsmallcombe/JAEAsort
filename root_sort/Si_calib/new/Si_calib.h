...
public :
	...
	vector< vector< TH2D*  >   > dE_2D_graphs;
...
Bool_t Si_calib::Notify()
{
   ...
   // user if needed. The return value is currently not used.
	if(fChain->GetTree()){cout<<endl<<"Opening to Calibrate : "<<fChain->GetCurrentFile()->GetName();

		
		
	// 		string outtitle=fChain->GetCurrentFile()->GetName();
	// 	outtitle=path+"si_cal/"+outtitle+"_sical.root";
	// 	
	// 				string basename
	// 			string mwpc_data_string=data_path+"y1y2/"+instr;
	// 			string si_data_string=data_path+"si/extra_"+instr;
	// 			string si_calibration_string=data_path+"si_cal/extra_"+basename+"_sical.root";	
